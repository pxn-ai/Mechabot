...
<head>
    ...
    <style>
        *, *::before, *::after { box-sizing: border-box; }
        :root {
            --bg-void: #03050a; --accent-cyan: #00f0ff; --accent-violet: #8a2be2;
            --danger-red: #ff2a55; --tech-green: #39ff14; --amber: #ffb800;
            --glass-bg: rgba(10, 15, 30, 0.4); --glass-border: rgba(0, 240, 255, 0.15);
        }
        body, html {
        ...
        @keyframes breathTech { 0%, 100% { opacity: 0.3; } 50% { opacity: 1; } }
        @keyframes lineBreath { 0% { opacity: 0.1; } 100% { opacity: 0.5; } }
        @keyframes spinRev { 100% { transform: rotate(-360deg); } }
        @keyframes emitPulse { 0% { transform: scale(1); opacity: 0.6; } 100% { transform: scale(2.5); opacity: 0; } }
        @keyframes textGlow { 0% { filter: brightness(1); } 100% { filter: brightness(1.3); } }
        ...
        .control-zone::after {
            content: ''; position: absolute; inset: 16px; border: 1px dashed rgba(255,255,255,0.03);
            border-radius: 24px; pointer-events: none; transition: all 0.3s;
        }
        ...
        .zone-label {
            font-size: 0.8rem; font-weight: 900; letter-spacing: 5px;
            text-transform: uppercase; writing-mode: vertical-rl;
            color: rgba(255,255,255,0.04); pointer-events: none;
        }
        .control-zone.active .zone-label { color: rgba(255,255,255,0.15); }
        .left-zone { border-right: 1px solid rgba(255,255,255,0.04); }
        .right-zone { border-left: 1px solid rgba(255,255,255,0.04); }

        /* ── Central HUD ── */
        .hud-center {
            width: 300px; min-width: 280px; height: 100%; flex-shrink: 0;
        ...
        .hud-title {
            font-size: 0.8rem; font-weight: 900; letter-spacing: 3px; text-transform: uppercase;
            background: linear-gradient(90deg, var(--accent-cyan), var(--accent-violet));
            -webkit-background-clip: text; color: transparent; margin-bottom: 2px;
            animation: textGlow 2s ease-in-out infinite alternate;
        }
        .status-txt {
            font-size: 0.5rem; font-weight: 700; color: rgba(255,255,255,0.35);
            letter-spacing: 1px; margin-bottom: 8px; display: flex; align-items: center; gap: 4px;
        }
        .status-dot { width: 5px; height: 5px; border-radius: 50%; background: var(--tech-green); box-shadow: 0 0 8px var(--tech-green); animation: breathTech 2s infinite; }
        ...
        .compass-ring::after {
            ...
            border: 1px dashed rgba(0,240,255,0.15); animation: spinRev 20s linear infinite;
        }
        .compass-needle {
            position: absolute; top: 6px; left: 50%; width: 2px; height: 52px;
        ...
        .compass-needle::before {
            ...
            background: var(--danger-red); box-shadow: 0 0 8px var(--danger-red); border-radius: 2px;
        }
        ...
        .heading-row {
            display: flex; justify-content: center; align-items: baseline; gap: 2px; margin-top: 4px;
        }
        .heading-val { font-size: 1.2rem; font-weight: 900; color: #fff; font-variant-numeric: tabular-nums; }
        .heading-unit { font-size: 0.6rem; color: var(--accent-cyan); }

        /* ── IMU Gauges ── */
        .gauge-row { display: flex; gap: 8px; justify-content: center; }
        .gauge {
            text-align: center; flex: 1; padding: 4px;
            border-radius: 8px; background: rgba(0,0,0,0.3);
        }
        .gauge-val { font-size: 0.9rem; font-weight: 900; font-variant-numeric: tabular-nums; }
        .gauge-lbl { font-size: 0.4rem; text-transform: uppercase; letter-spacing: 1px; color: rgba(255,255,255,0.35); }
        .g-pitch .gauge-val { color: var(--accent-cyan); }
        .g-roll .gauge-val { color: var(--accent-violet); }
        .g-gyro .gauge-val { color: var(--amber); }

        /* ── Buttons ── */
        .switch-row { display: flex; gap: 6px; }
        .toggle-btn {
            flex: 1; padding: 8px 6px; border-radius: 8px; background: rgba(0,0,0,0.3);
            border: 1px solid rgba(255,255,255,0.08); color: rgba(255,255,255,0.5);
            font-size: 0.55rem; font-weight: 800; text-transform: uppercase; cursor: pointer;
            transition: all 0.15s; position: relative; overflow: hidden; outline: none;
        }
        .toggle-btn.active {
        ...
        .btn-stop {
            ...
            background: rgba(255,42,85,0.1); border: 1px solid rgba(255,42,85,0.25);
            color: var(--danger-red); font-size: 0.9rem; font-weight: 900; letter-spacing: 4px;
            cursor: pointer; text-transform: uppercase; transition: all 0.1s; outline: none;
        }
        .btn-stop:active { background: var(--danger-red); color: #fff; transform: scale(0.96); }

        /* ── Slider ── */
        .slider-wrap { width: 100%; margin-bottom: 4px; }
        .slider-label { font-size: 0.45rem; color: rgba(255,255,255,0.35); text-transform: uppercase; letter-spacing: 1.5px; display: block; text-align: center; margin-bottom: 4px; }
        input[type="range"] {
        ...
        .pid-field input {
            ...
            border-radius: 6px; color: #fff; font-size: 0.7rem; font-weight: 700;
            padding: 6px; text-align: center; outline: none; font-family: inherit;
            width: 100%;
        }
        .pid-field input:focus { border-color: var(--accent-cyan); }
        .pid-apply {
            grid-column: span 2; padding: 8px; border-radius: 8px; margin-top: 2px;
            background: rgba(138,43,226,0.12); border: 1px solid rgba(138,43,226,0.25);
            color: var(--accent-violet); font-size: 0.6rem; font-weight: 800;
            text-transform: uppercase; letter-spacing: 1px; cursor: pointer; outline: none;
        }
        .pid-apply:active { transform: scale(0.95); background: rgba(138,43,226,0.25); }

        /* ── Joystick Visuals ── */
        .stick-base {
            position: absolute; width: 130px; height: 130px; border-radius: 50%;
            border: 2px dashed rgba(255,255,255,0.12);
            background: radial-gradient(circle, rgba(255,255,255,0.015) 20%, transparent 60%);
        ...
        #baseL .stick-core { background: radial-gradient(circle at 35% 35%, #fff, var(--accent-cyan)); box-shadow: 0 0 25px rgba(0,240,255,0.4); }
        #baseR .stick-core { background: radial-gradient(circle at 35% 35%, #fff, var(--accent-violet)); box-shadow: 0 0 25px rgba(138,43,226,0.4); }
        ...
        body.driving .hud-center { box-shadow: 0 0 50px rgba(0,240,255,0.12); }
        body.driving .hud-module { border-color: rgba(0,240,255,0.2); }

        /* ── Collapsible toggle ── */
        .collapse-toggle {
            width: 100%; background: none; border: none; color: rgba(255,255,255,0.25);
            font-size: 0.45rem; font-weight: 700; text-transform: uppercase; letter-spacing: 2px;
            cursor: pointer; padding: 4px; margin-bottom: 4px; outline: none;
        }
        .collapse-toggle:active { color: rgba(255,255,255,0.5); }
        .collapsible { transition: max-height 0.3s, opacity 0.3s; overflow: hidden; }
        .collapsible.collapsed { max-height: 0 !important; opacity: 0; margin: 0; padding: 0; }

        @media (max-width: 700px) {
            .hud-center { width: 240px; min-width: 220px; padding: 8px; }
            .compass-ring { width: 50px; height: 50px; }
            .compass-needle { top: 4px; height: 42px; }
            .heading-val { font-size: 1rem; }
        }
    </style>
</head>
<body>
    ...
    <div class="layout">
        <!-- LEFT: TRANSLATE -->
        <div class="control-zone left-zone" id="zoneL"><div class="zone-label">Translate</div></div>

        <!-- CENTER HUD -->
        <div class="hud-center">
            <div class="hud-title">&#9889; Vector Core</div>
            <div class="status-txt"><span class="status-dot"></span><span id="statusLbl">IDLE</span>&ensp;|&ensp;<span id="imuStatus">IMU:--</span></div>
            ...
            <div class="hud-module">
                ...
                <div class="gauge-row">
                    <div class="gauge g-pitch"><div class="gauge-val" id="pitchVal">0.0</div><div class="gauge-lbl">Pitch</div></div>
                    <div class="gauge g-roll"><div class="gauge-val" id="rollVal">0.0</div><div class="gauge-lbl">Roll</div></div>
                    <div class="gauge g-gyro"><div class="gauge-val" id="gyroZVal">0.0</div><div class="gauge-lbl">Gyro Z&deg;/s</div></div>
                </div>
            </div>
        ...
        <!-- RIGHT: YAW -->
        <div class="control-zone right-zone" id="zoneR"><div class="zone-label">Rotation</div></div>
    </div>
    ...
    <script>
        ...
        const MOCK = false;
        const J_RADIUS = 60;
        let vx = 0, vy = 0, wz = 0, cruiseMode = false, activePointers = {};

        const $ = id => document.getElementById(id);
        const baseL = $('baseL'), nubL = $('nubL'), baseR = $('baseR'), nubR = $('nubR');
        ...
        window.emergencyStop = () => {
            ...
            fetch('/stop').catch(()=>{});
            endStick(baseL, nubL, zoneL); endStick(baseR, nubR, zoneR);
        };
        ...
        // ══ JOYSTICKS ══
        function startStick(base, zone, x, y) {
            base.style.display = 'block'; base.style.left = x+'px'; base.style.top = y+'px';
            nubL.style.transition = 'none'; nubR.style.transition = 'none';
            zone.classList.add('active'); base.classList.add('emitting');
        }
        function endStick(base, nub, zone) {
            base.classList.remove('emitting'); base.style.display = 'none';
        ...
        document.body.addEventListener('pointerdown', e => {
            if (e.target.closest('.hud-center')) return;
            let isLeft = e.clientX < window.innerWidth / 2;
            startStick(isLeft ? baseL : baseR, isLeft ? zoneL : zoneR, e.clientX, e.clientY);
            activePointers[e.pointerId] = { left: isLeft, sx: e.clientX, sy: e.clientY };
            e.target.setPointerCapture(e.pointerId);
        });
        document.body.addEventListener('pointermove', e => {
            let p = activePointers[e.pointerId]; if (!p) return;
            let dx = e.clientX - p.sx, dy = e.clientY - p.sy;
            let dist = Math.sqrt(dx*dx + dy*dy);
            if (dist > J_RADIUS) { dx = (dx/dist)*J_RADIUS; dy = (dy/dist)*J_RADIUS; }
            (p.left ? nubL : nubR).style.transform = `translate(calc(-50% + ${dx}px), calc(-50% + ${dy}px))`;
            if (p.left) { vx = dx/J_RADIUS; vy = -dy/J_RADIUS; }
            else { wz = dx/J_RADIUS; }
        });
        function pEnd(e) {
            let p = activePointers[e.pointerId]; if (!p) return;
            endStick(p.left?baseL:baseR, p.left?nubL:nubR, p.left?zoneL:zoneR);
            if (!cruiseMode || !p.left) { if (p.left) { vx=0; vy=0; } else { wz=0; } }
            delete activePointers[e.pointerId];
        }
        document.body.addEventListener('pointerup', pEnd);
        document.body.addEventListener('pointercancel', pEnd);
        ...
        // Load PID values from ESP on connect
        api('/pid', d => {
            if (d) {
