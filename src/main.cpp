...
{
    if (coreMode == MODE_CALIBRATING)
        return "calibrating";
    if (coreMode == MODE_PRECISE_TURN)
        return "turning";
    if (targetVx == 0 && targetVy == 0 && targetWz == 0)
        return "idle";
    return "driving";
}

void sendTelemetryResponse(AsyncWebServerRequest *r)
{
    char j[320];
    float heading = getHeading();
    float compassHeading = getCompassHeading();
    snprintf(j, sizeof(j),
             "{\"heading\":%.1f,\"fusedHeading\":%.1f,\"compass\":%.1f,\"rawHeading\":%.1f,"
             "\"pitch\":%.1f,\"roll\":%.1f,\"gyroZ\":%.1f,\"compassAssist\":%s,"
             "\"calibrated\":%s,\"imuReady\":%s,\"compassReady\":%s,\"imu\":%s,\"mode\":\"%s\"}",
             heading,
             heading,
             compassHeading,
             compassHeading,
...
bool executePreciseTurn(float dt)
{
    ...
    if (error > 0)
        driveKinematics(0, 0, wzNorm, 255);
    else
        driveKinematics(0, 0, -wzNorm, 255);
...
void setup()
{
    ...
    server.on("/heading", HTTP_GET, [](AsyncWebServerRequest *r)
              { sendTelemetryResponse(r); });

    // ── IMU telemetry ──
    server.on("/imu", HTTP_GET, [](AsyncWebServerRequest *r)
              { sendTelemetryResponse(r); });
